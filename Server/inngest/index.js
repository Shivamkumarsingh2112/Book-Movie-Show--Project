...
// Inngest Function to cancel booking and release seats of show after 10 minutes of booking created if pay,ment is not made
...
const sendBookingConfirmationEmail = inngest.createFunction(
    ...
    async ({event, step}) => {
        ...
        await sendEmail({
            ...
            <p>
            <p>Enjoy the show! <p>
            <p>Thanks for booking with us!<br/>- Quick Show Team<p>
            </div>`
        })
    }
        ...
        const results = await step.run('send-all-reminders', async () => {
            return await Promise.allSettled(
                reminderTasks.map(task => sendEmail({
                    ...
                    <br/>
                    <p>enjoy the show<br/> quick show  team <p>
                    </div>`
                }))
            )
        })
...
const sendNewShowNotifications = inngest.createFunction(
    ...
    async ({event}) =>{
        ...
        for(const user of users){
            ...
            const body= ` we just added a new show to our library`;
