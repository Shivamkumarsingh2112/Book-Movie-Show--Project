  ...
  return !loading ? (
    <>
      ...
      <div className='relative flex flex-wrap gap-4 mt-6'>
        ...
        <div className='flex flex-row gap-8 w-lvh '>
          {dashboardCards.map((card,index) => (
            <div key={index} className='flex items-center justify-between px-4 py-3 bg-primary/10 border border-primary/20 rounded-md-max-w-50 w-full'>
              <div>
                ...
                <p className='text-2xl font-medium mt-2'>{card.value}</p>
                </div>
       ...
       <div className='relative flex flex-wrap gap-15 mt-4 max-w-max '>
        <BlurCircle top="100px" left="-10px"/>
        {dashboardData.activeShows.map((show) => (
          <div key={show._id} className='w-65 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:-translate-y-1 transition duration-300'>
            <img src={image_base_url + show.movie.poster_path} alt='' className='h-60 w-full object-cover'/>
            <p className='font-medium p-2 truncate'>{show.movie.title}</p>
            <div className='flex items-center justify-between px-2'>
              <p className='text-lg font-medium'>{currency} {show.showPrice}</p>
              <p className='flex items-center gap-1 text-2xl text-gray-400 mt-1 pr-1'>
              <StarIcon className='w-4 h-4 text-primary fill-primary'/>
              {show.movie.vote_average.toFixed(1)}
              </p>
              </div>
              <p className='px-2 pt-2 text-2xl text-gray-500'>{dateFormat(show.showDateTime)}</p>
              </div>
        ))}
        </div>
